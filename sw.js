// ═══════════════════════════════════════════════════════════════════════════
// SERVICE WORKER — AR Tracker  v5  (offline-first + précache images trackées)
// ═══════════════════════════════════════════════════════════════════════════

const CACHE_SHELL   = 'ar-shell-v5';
const CACHE_IMAGES  = 'ar-images-v5';
...
self.addEventListener('fetch', function(event) {
  ...
  // ── Network-first : overrides.json (mis à jour fréquemment) ──────────────
  if (url.includes('overrides.json')) {
    event.respondWith(
      fetch(event.request, { cache: 'no-store' })
        .then(function(response) {
          if (response && response.status === 200) {
            caches.open(CACHE_SHELL).then(function(c) { c.put(event.request, response.clone()); });
          }
          return response;
        })
  ...
  // Navigation requests → always serve index.html from cache or network
  if (event.request.mode === 'navigate') {
    event.respondWith(
      caches.open(CACHE_SHELL).then(function(cache) {
        return cache.match('./index.html').then(function(cached) {
          var networkFetch = fetch('./index.html').then(function(response) {
  ...
  if (
    ...
    url.includes('static.tcgcollector.com')
  ) {
    event.respondWith(
      caches.open(CACHE_IMAGES).then(function(cache) {
